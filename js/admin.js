  ...
  function resumeAnnonce(kind, item) {
    ...
    if (kind === 'villa') {
      return { titre: item.name, sous: item.location, lignes: [
        l(item.priceUnit === 'chambre' ? 'Tarif par chambre et par nuit' : 'Tarif par nuit', money(item.pricePerNight)), l('Forfait week-end', item.weekendPackage ? money(item.weekendPackage) : ''),
...
Les téléphones qui l'ont déjà reçue la gardent.`)) return;
  ...
  function normalizeVilla(v, old, gallery) { const price = Number(v.pricePerNight || 0); return { ...old, facebook: v.facebookPresent ? v.facebook === 'yes' : (old.facebook ?? null), id:v.id.trim().toLowerCase(), name:v.name.trim(), category:v.category, categoryLabel: libelleRef(trouverRef('categories', v.category)) || v.category, environment: CADRES_VILLA.some(c => c.value === v.environment) ? v.environment : cadreDeVilla({ ...old, ...v }), status:v.status, visible:v.visible==='yes', featured:v.featured==='yes', badgeId: v.badgeId || '', badge: libelleRef(trouverRef('badges', v.badgeId)), localisationId: v.localisationId || '', localisationPrecision: v.localisationId ? String(v.localisationPrecision || '').trim() : '', location: adresseAffichee(v.localisationId, v.localisationPrecision, old.location), equipements: Array.isArray(v.equipements) ? v.equipements : [], pricePerNight:price, priceUnit: v.priceUnit === 'chambre' ? 'chambre' : 'villa', priceEuro:Math.round(price/655.957), weekendPackage:Number(v.weekendPackage || 0), capacity:Number(v.capacity), bedrooms:Number(v.bedrooms), bathrooms:Number(v.bathrooms), beds:v.beds, tagline:v.tagline, description:v.description, images:gallery.slice(0,12), features:lines(v.features), highlights:lines(v.highlights), rating:old.rating || 5, reviewsCount:old.reviewsCount || 0 }; }
